...
NomFichResultados = 'ResultadosCuantificacionGNG';
NomFichEvaluaciones = 'EvaluacionesCuantificacionGNG';
NomFichModelos = 'ModelosCuantificacionGNG';
RutaImagenes = './images/';
...
%     Muestras = s.m;
    ...
    for NdxMaxNeurons=1:length(MaxNeurons),
        ...
        for NdxRepeticion=1:NumEntrenamientos,
            ...
            if ~isempty(Evaluaciones{NdxRepeticion,NdxMaxNeurons,NdxDataset}),
                ...
                while Hecho==0
                    try
                        tic;
                        [Modelo] = TrainGNG(Muestras,MyMaxNeurons,Lambda,EpsilonB,EpsilonN,Alpha,AMax,D,NumPasos);
                        CpuTime = toc;
        ...
        ImgProt = GetPrototypesImg(Centroids,Winners,size(ImgOriginal));
        fig = figure;
        set(fig,'name',['GNG ' num2str(MyMaxNeurons) ' Neurons ' NomFich]);
        set(fig,'NumberTitle','off');
        imshow(ImgProt);
        saveas(gcf,['GNG_' num2str(MyMaxNeurons) '_' strrep(NomFich,'.tiff','') '.fig'],'fig');   
        ...
        imshow(ImgDif);  
        saveas(gcf,['GNG_Diff_' num2str(MyMaxNeurons) '_' strrep(NomFich,'.tiff','') '.fig'],'fig');
    end % for MaxNeruons
    close all
end % for dataset
