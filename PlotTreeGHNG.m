function [PlotHandle]=PlotTreeGHNG(Model,GlobalMean,Uq,NumRowsImg,NumColsImg,ImageSize)
% Plot a GNF model as a tree, with the mean vectors at the nodes
% This is for input data which has been previously reduced by global PCA
...
%   Uq=Eigenvectors of the global PCA
...
[Images Parents Coords] = CreateParentList(Model,GlobalMean,Uq,NumColsImg,NumRowsImg,...
...
function [ImagesOut,ParentsOut,CoordsOut] = CreateParentList(Model,GlobalMean,Uq,NumColsImg,NumRowsImg,ImagesIn,ParentsIn,CoordsIn,NdxMyParent,MaxLevel,ImageSize)
...
%     ImagesOut{end+1} = rot90(fliplr(reshape(GlobalMean+Uq*Model.Means(:,NdxNeuro),NumColsImg,NumRowsImg)));  
    ImagesOut{end+1} = reshape(GlobalMean+Uq*Model.Means(:,NdxNeuro),NumColsImg,NumRowsImg);
    CoordsOut(:,end+1) = [CoordsOut(1,NdxMyParent)+ImageSize;1-(1+size(CoordsOut,2))*ImageSize];
    if (isfield(Model,'Child')),
        [ImagesOut,ParentsOut,CoordsOut] = CreateParentList(Model.Child{NdxNeuro},GlobalMean,Uq,NumColsImg,NumRowsImg,ImagesOut,ParentsOut,CoordsOut,numel(ParentsOut),MaxLevel-1,ImageSize);   
    end
end
