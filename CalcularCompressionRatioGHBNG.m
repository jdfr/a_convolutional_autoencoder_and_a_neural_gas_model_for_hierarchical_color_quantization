...
load('ModelosCuantificacionDivergenciasBregman.mat','Modelos');

[numRows, numCols] = size(Modelos)

for i = 1:4
    for j = 1:numRows
        modelillo = Modelos{j,i};
        Centroids = GetCentroidsGHBNG(Modelos{j,i});
        NumNeurons = size(Centroids,2);
        CompressedByteSize = NumPixeles*(ceil(log2(NumNeurons))/8);
        CR = ByteSize/CompressedByteSize;
        fprintf('%s Compression Ratio (CR): %6.2f (%d/%d) (Neurons=%d)\n', Datasets{i}, CR,ByteSize,CompressedByteSize,NumNeurons);
    end
end
