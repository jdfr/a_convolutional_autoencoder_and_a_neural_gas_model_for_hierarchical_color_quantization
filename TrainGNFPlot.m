function [Model]=TrainGNFPlot(Samples,MaxUnits,Lambda,EpsilonB,EpsilonN,Alpha,AMax,D,NumSteps,NomFich)
% Train a Growing Neural Forest (GNF) neural network and plot each step
% The GNF is similar to the GNG, but it computes a spanning tree for each
...
PlotGNFModel2DDistribution(Model,NomFich,1,0);
% pause
...
%     PlotGNFModel2DDistribution(Model,NomFich,CreatedNeuron,NdxStep);
    PlotGNFModel2DDistribution(Model,NomFich,1,NdxStep);
    CreatedNeuron = 0;
    close all;
end
PlotGNFModel2DDistribution(Model,NomFich,1,NdxStep);
